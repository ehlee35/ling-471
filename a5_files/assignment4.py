...
def my_naive_bayes(data, column_label):
    ROUND = 4
    ...
    X_train = train_data[column_label]
    y_train = train_data.label
    X_test = test_data[column_label]
    ...
    dict = {'TRAIN': {'accuracy': round(accuracy_train, ROUND), 'precision_pos': round(precision_pos_train, ROUND),
                      'recall_pos': round(recall_pos_train, ROUND), 'precision_neg': round(precision_neg_train, ROUND),
                      'recall_neg': round(recall_neg_train, ROUND)},
            'TEST': {'accuracy': round(accuracy_test, ROUND), 'precision_pos': round(precision_pos_test, ROUND),
                      'recall_pos': round(recall_pos_test, ROUND), 'precision_neg': round(precision_neg_test, ROUND),
                      'recall_neg': round(recall_neg_test, ROUND)}}
...
def main(argv):
    ...
    print(my_naive_bayes(data, 'review'))
