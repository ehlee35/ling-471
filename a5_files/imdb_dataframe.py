...
def createDataFrames(argv):
    ...
    # Do all the required preprocessing.

    for index, item in enumerate(train_pos):
        path_name = str(item).split("\\")

        data.append([path_name[-1], path_name[-2], path_name[-3]] + list(cleanFileContents(item)))

        if index % 100 == 0:
            print("Train POS: Processing directory 1 out of 4; file ", str(index), " out of 12500")

    for index, item in enumerate(train_neg):
        path_name = str(item).split("\\")

        data.append([path_name[-1], path_name[-2], path_name[-3]] + list(cleanFileContents(item)))

        if index % 100 == 0:
            print("Train NEG: Processing directory 2 out of 4; file ", str(index), " out of 12500")

    for index, item in enumerate(test_pos):
        path_name = str(item).split("\\")

        data.append([path_name[-1], path_name[-2], path_name[-3]] + list(cleanFileContents(item)))

        if index % 100 == 0:
            print("Test POS: Processing directory 3 out of 4; file ", str(index), " out of 12500")

    for index, item in enumerate(test_neg):
        path_name = str(item).split("\\")

        data.append([path_name[-1], path_name[-2], path_name[-3]] + list(cleanFileContents(item)))

        if index % 100 == 0:
            print("Test NEG: Processing directory 4 out of 4; file ", str(index), " out of 12500")
