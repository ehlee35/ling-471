...
def createDataFrame(argv):
    ...
    for index in range(1, len(argv)):
        list_of_files = list(Path(argv[index]).glob('*.txt'))

        for file_path in list_of_files:
            path_name = str(file_path).split("\\")

            data.append([path_name[-1], path_name[-2], path_name[-3], cleanFileContents(file_path)])

    # Try to create a list of lists, for example, as illustrated above.
    # Consider writing a separate function which takes a filename and returns a list representing the review vector.
    # This will make your code here cleaner.
    # HINT: When you are trying to get the pos/neg value and the test/train value,
    ...
    # but then make sure to make appropriate changes in ehlee35_assignment4.py.
